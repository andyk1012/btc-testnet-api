...
def cmd_send(args):
    ...
    fr = args.fee_rate if args.fee_rate else fee_rate_sat_vb()
    ...
    for u in utxos_sorted:
        selected.append(u)
        total_in += u.value
        # optimistic fee estimate with 2 outputs
    ...
    # final fee calc
    n_out = 2
    vbytes = estimate_vbytes_p2pkh(n_in, n_out)
    fee = fr * vbytes
    ...
    # Sign each input (legacy sighash)
    sk = SigningKey.from_string(priv, curve=SECP256k1)
    final_txins = []
    for idx, u in enumerate(selected):
        # For sighash, scriptSig for this input must be previous output scriptPubKey; others empty.
        tmp_ins = []
...
def build_parser():
    ...
    s.add_argument("--fee-rate", type=int, help="Fee rate in sat/vB (optional)")
